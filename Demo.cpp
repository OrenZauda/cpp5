...
int main() {
	...
	for (int i: chain<range<int>,range<int>>(range<int>(1,4), range<int>(5,8)))
		cout << i;    // prints 123567
	cout << endl << endl << "Chain of a range and a string: " << endl;
	for (char i: chain<range<char>,string>(range<char>('a','e'), string("hello")))
		cout << i;    // abcdhello

	cout << endl << endl << "Zip a range of ints and a string (must be of the same size)" << endl;
	for (auto pair: zip<range<int>,string>(range<int>(1,6), string("hello")))
